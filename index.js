...
const OrderChannels = require('./players').OrderChannels;
const db = require('./db');
const herokuClient = require('./herokuClient');

playerQuery = `SELECT * FROM public."Users" WHERE channel = 'elandi'`;
db.query(playerQuery).then(p => {
    mapsQuery = `SELECT * FROM public.maps`;
    db.query(mapsQuery).then(m => {
        DBmaps = m.rows;
        DBplayers = p.rows;
        OrderChannels(DBplayers, options);
        const client = new tmi.client(options);
        client.connect();

        UpdateMapNames(DBmaps);
        ...
        client.on("chat", (channel, userstate, message, self) => {
            ...
            async function SearchPlayer(searchTerm) {
                var aliasPerson = FindPlayer(searchTerm, DBplayers);
                if (aliasPerson) {
            ...
            if (CommandIs('!stime') || CommandIs('!dtime')) {
                ...
                var person = FindPlayerFromChannel(channel, DBplayers).aliases;
                ...
                if (commandMap == undefined) {
                    SearchPlayer(FindPlayerFromChannel(channel, DBplayers).aliases).then(p =>
                        SearchPlayerMap(p)
                            .then(map =>
            ...
            if (CommandIs('!sbtime') || CommandIs('!dbtime')) {
                ...
                if (!isNaN(+commandMap) && message.split(' ')[3] == undefined) {
                    var person = FindPlayerFromChannel(channel, DBplayers);
                    ...
                    if (zoneIndex && !isNaN(-zoneIndex)) {
                        ...
                        if (searchTerm == '') {
                            searchTerm = FindPlayerFromChannel(channel, DBplayers).aliases;
                        }
            ...
            if (CommandIs('!restart') && "#" + userstate.username === channel) {
                herokuClient.restart();
            }
            if (CommandIs('!addsoldiermaps') && "#" + userstate.username === channel) {
                //intendedMaps["demo"].forEach(map => {
                //insertMapQuery = `INSERT INTO public.maps(name, class) VALUES ('${map}','demo')`
                //db.query(insertMapQuery);
                //})
            }
            if (CommandIs('!addaliastempusidchannel') && userstate.username === 'elandi') {
                alias = message.split(' ')[1];
                id = message.split(' ')[2];
                Twitchchannel = message.split(' ')[3];
                if (!alias) { client.say(channel, 'error, no alias found'); return }
                if (!id) { client.say(channel, 'error, no id found'); return }
                if (!Number.isInteger(+id)) { client.say(channel, 'id is not a number'); return }
                if (!Twitchchannel) { client.say(channel, 'error, no channel found'); return }
                Insertquery = `INSERT INTO public."Users"(aliases, id, channel) VALUES ('${alias}','${id}','${Twitchchannel}')`
                db.query(Insertquery)
                    .then(res => {
                        if (res.rowCount > 0) {
                            client.say(channel, `${alias}, ${id}, ${Twitchchannel} was added`)
                        } else {
                            client.say(channel, `${Twitchchannel} not found or not removed`);
                        }
                    }).catch(err => {
                        console.log(err)
            ...
            if (CommandIs('!removechannel') && userstate.username === 'elandi') {
                let Twitchchannel = message.split(' ')[1];
                if (!Twitchchannel) { client.say(channel, 'error, no channel found'); return }
                deleteQuery = `DELETE FROM public."Users" WHERE channel = '${Twitchchannel}'`
                db.query(deleteQuery).then(res => {
                    if (res.rowCount > 0) {
                        client.say(channel, `${Twitchchannel} removed`);
                    } else {
                        client.say(channel, `${Twitchchannel} not found`);
                    }
                }).catch(err => {
                    console.log(err)
                    client.say(channel, err.message);
            ...
            if (CommandIs('!tierlist') && "#" + userstate.username === channel) {
                db.query(`SELECT * FROM public.maps WHERE class IS NULL`).then(m => {
                    ...
                    m.rows.forEach(nulls => {
                        client.say(channel, nulls.name);
                    })
                })
            }
            if (CommandIs('!tieradd') && userstate.username === 'elandi') {
                ...
                if (classType == 'soldier' || classType == 'demo' || classType == 'both') {
                    updateQuery = `UPDATE public.maps SET class = '${classType}' WHERE name = '${map}'`
                    DBmaps.push({ name: map, class: classType });
            ...
            if (CommandIs('!tierremove') && userstate.username === 'elandi') {
                let map = ClosestsName(commandMap);
                deleteQuery = `DELETE FROM public.maps WHERE name = '${map}'`
                db.query(deleteQuery).then(res => {
                    if (res.rowCount > 0) {
            ...
            if (CommandIs('!tierremoveexact') && userstate.username === 'elandi') {
                deleteQuery = `DELETE FROM public.maps WHERE name = '${commandMap}'`
                db.query(deleteQuery).then(res => {
                    if (res.rowCount > 0) {
            ...
            */
            ...
            if (CommandIs('!random')) {
                if (commandMap == undefined) {
                    ...
                    switch (commandMap) {
                        case 's': case 'soldier':
                            client.say(channel, Random.ClassMap("soldier", DBmaps))
                            break;
                        case 'd': case 'demo': case 'demoman':
                            client.say(channel, Random.ClassMap("demo", DBmaps))
                            break;
                        case 'b': case 'both':
